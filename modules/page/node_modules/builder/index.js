...
Builder.prototype.cacheZones = function(){
	...
	forOwn(blocks, function(block, key){
		...
		el = self.blockWrapper.querySelector('[data-block-type=' + key + ']');
		el.classList[disabled ? 'add' : 'remove']('is-disabled');
	});
};
...
Builder.prototype.setEvents = function(){
	var self = this, isHeader, hoveredBlock;

	this.blockWrapper.addEventListener('mouseover', function(e){
		var block = getClosest(e.target, 'li');

		if (hoveredBlock && hoveredBlock !== block){
			hoveredBlock.classList.remove('is-hovered');
	...
	this.blockWrapper.addEventListener('mouseleave', function(e){
		if (hoveredBlock){
			hoveredBlock.classList.remove('is-hovered');
			hoveredBlock = undefined;
		}
	...
	document.body.addEventListener('dragover', function(e){
		...
		var zone = getClosest(e.target || e.srcElement, '[data-zone]');
		if (!zone){
			if (self.hoverZone){
	...
	this.template.addEventListener('dragenter', function(e){
		...
		var target = e.target || e.srcElement;
		var zone = getClosest(target, '[data-zone]');
		...
		var block = getClosest(target, '.block');
		if (!block) return;

		var placeholderIndex = indexOf(zone.childNodes, self.placeholder);
		var blockIndex = indexOf(zone.childNodes, block);

		if (placeholderIndex === -1) return;

		zone.insertBefore(self.placeholder, placeholderIndex > blockIndex ? block : block.nextSibling);
	});

	this.template.addEventListener('mousedown', function(e){
		var target = e.target || e.srcElement;
		if (getClosest(target, '.block--header')){
			isHeader = true;
		}
	});
...
Builder.prototype.setTemplate = function(name){
	...
	if (this.templateName){
		this.template.classList.remove('template--' + this.templateName);
	}
	this.templateName = name;
	this.template.classList.add('template--' + name);
